...
def generate_llm_audience_segment_insights(
    ...
    if f"{state_key}_Selected_Persona" in st.session_state:
        options=st.session_state[f"{state_key}_Personas_File"].iloc[:,0].unique().tolist()
    
    if f"{state_key}_Personas_File" not in st.session_state:
        options=["Select Persona File ..."]
    else:
        options=["Select Persona ..."] + st.session_state[f"{state_key}_Personas_File"].iloc[:,0].unique().tolist()
        ...
        if f"{state_key}_Personas_File" in st.session_state:
            personas_option = st.selectbox(
                label=("Select one of the Personas "
                    "and continue your work. This persona will be selected to create a campaign brief."),
                options=options,
                key=f"{state_key}_Selected_Persona_Option", 
                on_change=_set_selected_persona)
