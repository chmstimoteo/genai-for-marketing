...
class GoogleTrends:
    ...
    def _suggestions(self, interest_keyword):
        # do the request
        result = self.pytrend.suggestions(interest_keyword)
        return result
    ...
    def _interest_over_time(self):
        result = self.pytrend.interest_over_time()
        return result
    ...
    def _related_queries(self):
        # do the request
        result = self.pytrend.related_queries()
        return result
    ...
    def get_search_interests_queries(
        self,
        interest_keyword: str, 
        min_trends_date: str,
        max_trends_date: str,
        state_key_interests: str,
        state_key_queries: str
    ):
        ...
        if st.session_state[state_key_interests].empty:
            try:
                interests_df = self._interest_over_time()
                interests_df.drop(columns=['isPartial'], inplace=True)
                interests_df.index = pd.to_datetime(interests_df.index).date
                interests_df['moving_average'] = interests_df.rolling(window=7).mean()
                interests_df.sort_index(ascending=False, inplace=True)
                interests_df.drop(columns=[interest_keyword], inplace=True)
                st.session_state[state_key_interests] = interests_df.head(15).T
            except (TooManyRequestsError, ResponseError):
                pass
        st.write(
            "Below you find Google Trends interest over time "
            "expressed by a weekly moving average." 
        )
        st.dataframe(st.session_state[state_key_interests])

        if st.session_state[state_key_queries].empty:
            try:
                result = self._related_queries()
                related_queries_df = result[interest_keyword]['rising']
                related_queries_df.drop(columns=['value'], inplace=True)
                st.session_state[state_key_queries] = related_queries_df.head(15).T
            except (TooManyRequestsError, ResponseError):
                pass
        st.write(
            "Below you find the related search terms " 
            "ordered by the number of queries over the specified period of time."
        )
        st.dataframe(st.session_state[state_key_queries])
    ...
    def get_url(
        ...
        st.session_state[state_key_url] = \
            f"https://trends.google.com/trends/explore?date={min_trends_date}%20{max_trends_date}&geo={geo_dma}&q={search_query}".format(
